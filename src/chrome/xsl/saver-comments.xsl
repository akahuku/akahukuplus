...
<xsl:choose><xsl:when test="descendant::*[contains(@class,'email')]"><span class="cnw"><a href="mailto:{descendant::*[contains(@class,'email')]/text()}"><xsl:value-of select="descendant::*[@class='postdate']/text()"/></a><xsl:if test="descendant::*[@class='user-id']"><xsl:text> </xsl:text><xsl:value-of select="descendant::xhtml:span[@class='user-id']/text()"/></xsl:if></span></xsl:when><xsl:otherwise><span class="cnw"><xsl:value-of select="descendant::*[@class='postdate']/text()"/><xsl:if test="descendant::*[@class='user-id']"><xsl:text> </xsl:text><xsl:value-of select="descendant::xhtml:span[@class='user-id']/text()"/></xsl:if></span></xsl:otherwise></xsl:choose>
...
<xsl:element name="blockquote"><xsl:if test="descendant::*[contains(@class,'reply-image')]"><xsl:attribute name="style">margin-left:<xsl:value-of select="descendant::*[contains(@class,'reply-image')]/xhtml:a/xhtml:img/@width + 40"/>px;</xsl:attribute></xsl:if><xsl:apply-templates select="descendant::*[@class='email']"/><xsl:apply-templates select="descendant::*[@class='comment']"/></xsl:element>
	...
	<xsl:template match="*[@class='comment']//xhtml:div[not(@class)]|*[@class='comment']//xhtml:iframe"><xsl:copy-of select="."/></xsl:template>
</xsl:stylesheet>
