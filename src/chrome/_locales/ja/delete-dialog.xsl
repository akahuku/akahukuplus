...
-->
<!--
 * Copyright 2014-2026 akahuku, akahuku@gmail.com
 *
...
<div>
	...
	<div>
		<fieldset>
			<legend><label><input type="radio" name="evalmode" value="delete" checked="checked"/> 記事の削除</label></legend>
			<div><label>削除キー <input type="text" name="pwd" class="delete-key" value="" maxlength="8"/></label></div>
			<div><label>[<input type="checkbox" name="onlyimgdel" class="delete-only-image" value="on"/> 画像だけ消す]</label></div>
		</fieldset>
		<fieldset>
			<legend><label><input type="radio" name="evalmode" value="moderate"/> del の送信</label></legend>
			<select class="reason"></select>
		</fieldset>
		<div class="delete-status-wrap">- <span class="delete-status"><xsl:value-of select="dialog/count"/> 件リクエストします</span> -</div>
	</div>
	</xsl:otherwise>
	</xsl:choose>
</div>
