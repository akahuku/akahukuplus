	...
	<tr>
		<th>
			...
			<div class="item-desc">
				...
				<xsl:if test="@min"><span class="comma">, </span>Min: <xsl:value-of select="@min"/></xsl:if>
				<xsl:if test="@max"><span class="comma">, </span>Max: <xsl:value-of select="@max"/></xsl:if>
			</div>
		</th>
		<td>
			<xsl:choose>
