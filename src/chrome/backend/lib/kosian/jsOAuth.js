...
if (typeof data != 'object') return false;
                ...
                xhr = Request();
                typeof options.initTransport == 'function' && options.initTransport(xhr);
...
responseObject.status = xhr.status;
                ...
                if(appendQueryString || method == 'GET') {
                    ...
                    query = null;
                } else if(data instanceof global.Blob) {
                    // TBD
                    query = data;
                } else if(!withFile){
...
return btoa(signature);
    ...
    function Request() {
        ...
        if (typeof this.requestTransport == 'function') {
            XHR = this.requestTransport();
            if (XHR) {
                return XHR;
            }
        }
...
// vim:set ts=4 sw=4 fenc=UTF-8 ff=unix ft=javascript fdm=marker expandtab :
